...
var SQ_chooseBarbe = function()
{
	...
	var ll = cards.length;
	for(var i = 0; i<9;i++){
		SQ_temp = 'b_'+cards.eq(i).find('.barb-card-name').html().replace(/\D/g,'');
...
var SQ_setArmy 					= function()
{
	...
	var miecz = 0;
	var luk = 0;
	if(SQ_level == 2) 	kuc = 2000;
	if(SQ_level == 3) 	kuc = 3000;
	if(SQ_level == 4) 	kuc = 4000;
	if(SQ_level == 5) 	kuc = 15000;
	if(SQ_level == 6) 	kuc = 20000;
	if(SQ_level == 7)	kuc = 25000;
	if(SQ_level == 8)	kuc = 40000;
	if(SQ_level == 9) 	kuc = 50000;
	if(SQ_level == 10) 	kuc = 70000;
	if(SQ_level == 11) 	kuc = 100000;

	if($('.'+SQ_formation).length) $('.'+SQ_formation)[0].click();
	if($('.gp-card').hasClass('empty'))
	{
...
var pickGeneral					= function()
{

                xajax_viewAssignGeneral(
                    container.open({
                        saveName: 'modal',
                        load: function(name) {
                            

                             $('#attack-general').clone()
                                .removeAttr('id')
                                .appendTo('#overlayContent')
                                .css({ position: 'absolute' })
                                .position({ my: 'center', at: 'center', of: '#attack-general' })
                                .find('.change').remove();
                            container.position('modal', { my: 'left center', at: 'right+20 center', of: '#overlayContent > div' });
                        }
                    }),
                    { 'assignmentType': 1}
                );
            setTimeout(function(){
				$('#generalsCarousel').find('a')[3].click();
